...
export const updateUserProfileSchema = Joi.object().keys({
  name: Joi.string().min(3).max(120).label('User name'),
  phone: Joi.string().regex(/^\+375\((17|25|29|33|44)\)[0-9]{3}-[0-9]{2}-[0-9]{2}$/).label('Phone number'),
  isPasswordChange: Joi.boolean().required(),
  oldPassword: Joi.when('isPasswordChange', { is: true, then: Joi.string().regex(/^[a-zA-Z0-9]{3,30}$/).label('Old password') }),
  password: Joi.when('isPasswordChange', { is: true, then: Joi.string().regex(/^[a-zA-Z0-9]{3,30}$/).label('Password') }),
});
